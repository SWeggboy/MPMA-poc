<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <?php $is_search_results = is_search(); ?>
    <div class="relative pt-16 before:absolute before:top-0 before:left-0 before:h-px before:w-6 before:bg-zinc-950 after:absolute after:top-0 after:right-0 after:left-8 after:h-px after:bg-zinc-950/10">
        <div class="<?php echo $is_search_results ? 'relative' : 'relative lg:-mx-4 lg:flex lg:justify-end'; ?>">
            <div class="<?php echo $is_search_results ? 'pt-10 lg:w-full lg:pt-0' : 'pt-10 lg:w-2/3 lg:flex-none lg:px-4 lg:pt-0'; ?>">
            ...
            <a class="<?php echo $is_search_results ? 'wp-element-button !no-underline mt-8 inline-flex items-center justify-center' : '!no-underline mt-8 inline-flex rounded-full bg-zinc-950 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-zinc-800'; ?>" aria-label="<?php echo esc_attr(sprintf(__('Read more: %s', 'tailpress'), get_the_title())); ?>" href="<?php the_permalink(); ?>">
