...
<?php else: ?>
    <form method="GET" action="<?php echo get_bloginfo('url'); ?>" class="search-box-wrapper">
        <div class="search-box relative border-2 border-dark/40 rounded-sm overflow-hidden bg-white w-[38px] h-[38px] transition-[width] duration-300 ease-in-out">
            <input type="text" name="s" class="search-input absolute left-0 top-0 border-0 px-3 text-lg bg-transparent w-full h-[34px] opacity-0 outline-none pointer-events-none transition-opacity duration-300 ease-in-out" value="<?php echo get_search_query(); ?>" placeholder="<?php _e('Search'); ?>">
